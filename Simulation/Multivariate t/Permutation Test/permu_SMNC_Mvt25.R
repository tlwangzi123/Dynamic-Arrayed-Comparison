...
wrapper <- function(name){
  ...
  for(i in 1:ndim){
    ...
    nps=rmvt(1,mu=np.means,sigma = np.covs,df=25)[1,]
  ...
  for(i in 1:ndim){
    ...
    nps2=rmvt(1,mu=np.means2,sigma = np.covs2,df=25)[1,]
...
#################################################################################
nSim <- 1000
SMNC_permu_MultiT25 <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(SMNC_permu_MultiT25,file = 'SMNC_permu_MultiT25.rda')
