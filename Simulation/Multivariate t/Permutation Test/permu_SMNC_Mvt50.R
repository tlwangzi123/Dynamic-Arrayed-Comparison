...
wrapper <- function(name){
  ...
  for(i in 1:ndim){
    ...
    nps=rmvt(1,mu=np.means,sigma = np.covs,df=50)[1,]
  ...
  for(i in 1:ndim){
    ...
    nps2=rmvt(1,mu=np.means2,sigma = np.covs2,df=50)[1,]
...
#################################################################################
nSim <- 1000
SMNC_permu_MultiT50 <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(SMNC_permu_MultiT50,file = 'SMNC_permu_MultiT50.rda')
