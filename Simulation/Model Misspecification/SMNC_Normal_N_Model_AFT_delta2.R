...
visit.num2=group2[['nvisit']]
  ...
  #count = glm.nb(visits~z1+z2+z3+z4+z5+offset(log(St)), data = countdata)
  ...
  for(i in 1:ndim){
    ...
    nps2=mvrnorm(1,np.means2,np.covs2)
...
#    cat(i,'\n')
  ...
  result = cbind(rowMeans(res[,seq(1,by=4,length=ndim)]),rowMeans(res[,seq(2,by=4,length=ndim)]),
  rowMeans(res[,seq(3,by=4,length=ndim)]),rowMeans(res[,seq(4,by=4,length=ndim)]))
  return(result)
},
...
#################################################################################
nSim = 1000 # test_num
SMNC_Normal_N_AFT_delta2_ModelMisSpec <- sfLapply(1:nSim,wrapper)
#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(SMNC_Normal_N_AFT_delta2_ModelMisSpec,file = 'SMNC_Normal_N_AFT_delta2_ModelMisSpec.RData')
#################################################################################
