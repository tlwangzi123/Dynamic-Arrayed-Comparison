...
wrapper <- function(name){
  ...
  for(i in 1:ndim){
    ...
    nps=100-qgamma(pnorm(nps),25,1/2)
  ...
  for(i in 1:ndim){
    ...
    nps2=100-qgamma(pnorm(nps2),25,1/2)
...
#################################################################################
nSim <- 1000
SMNC_permu_Gamma25 <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(SMNC_permu_Gamma25,file = 'SMNC_permu_Gamma25.rda')
