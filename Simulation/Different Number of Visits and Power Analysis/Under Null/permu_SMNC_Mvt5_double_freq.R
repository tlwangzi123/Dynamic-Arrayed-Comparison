...
wrapper <- function(name){
  ...
  alphas=seq(0.001,0.1,by=0.0001)*2
  for(jj in 1:max(ceiling(pmin(exp_survtime,20-X2)*lambdas*2)+1)){
  ...
  for(i in 1:ndim){
    ...
    np.means2=rep(50,6*visit.num2[i])
    ...
    for(k in 1:visit.num2[i]){
      directions=rbind(directions,c(0,0,
                                    ifelse(sum(nps2.quad[(k*6-5):(k*6)]-MEANS.quad[(k*6-5):(k*6)])<0,-1,1),
                                    0))
      LMNCs=rbind(LMNCs,c(0,0,
                          (nps2.quad[1:(k*6)]-MEANS.quad[1:(k*6)])%*%inversed.covs.quad[[k]]%*%(nps2.quad[1:(k*6)]-MEANS.quad[1:(k*6)]),0))
    }
    diffs=LMNCs-rbind(rep(0,4),LMNCs[-visit.num2[i],])
    diffs=diffs*directions
    alphas=seq(0.001,0.1,by=0.0001)
    N_prect=ceiling(min(exp_survtime[i],20-X2[i])*lambdas[i]*2)+1
    ...
    for(jj in 1:length(alphas)){
      ...
      signal=rbind(signal, ifelse(colSums(diffs<cbind(0,0,MF.quad,0))>0,1,0))
    }
...
#################################################################################
nSim <- 1000
SMNC_permu_MultiT5_typeI_doubleQ <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(SMNC_permu_MultiT5_typeI_doubleQ,file = 'SMNC_permu_MultiT5_typeI_doubleQ.rda')
