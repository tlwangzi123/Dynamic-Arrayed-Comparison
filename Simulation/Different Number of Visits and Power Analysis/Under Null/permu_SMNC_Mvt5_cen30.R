...
wrapper <- function(name){
  ...
  for(jj in 1:max(ceiling(pmin(exp_survtime,30-X2)*lambdas)+1)){
  ...
  for(i in 1:ndim){
    ...
    N_prect=ceiling(min(exp_survtime[i],30-X2[i])*lambdas[i])+1
...
#################################################################################
nSim <- 1000
SMNC_permu_MultiT5_typeI_cen30 <- sfLapply(1:nSim,wrapper)

#################################################################################
#  Save the result to a folder on the cluster:
#################################################################################
save(SMNC_permu_MultiT5_typeI_cen30,file = 'SMNC_permu_MultiT5_typeI_cen30.rda')
