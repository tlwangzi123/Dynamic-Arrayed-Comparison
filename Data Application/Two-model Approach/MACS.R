...
np_hiv_labs=read.csv("C:\\Users\\Zander Wang\\Box\\ZhengWang\\MACS\\Legacy\\np_hiv_labs.csv")
pe=read.csv("C:\\Users\\Zander Wang\\Box\\ZhengWang\\MACS\\Legacy\\pe.csv")
# comorbid=read.csv("C:\\Users\\Zander Wang\\Box\\ZhengWang\\MACS\\Legacy\\comorbid.csv")[,c("MACSID","VISIT","DATEY")]
...
visitnum.z=NULL
time.z=NULL
for(i in unique(Z$MACSID)){
  visitnum.z=c(visitnum.z,nrow(Z[Z$MACSID==i,]))
  time.z=c(time.z,Z[Z$MACSID==i,"VISITY.x"][nrow(Z[Z$MACSID==i,])]-Z[Z$MACSID==i,"VISITY.x"][1])
}

freq.z=(visitnum.z[time.z>0]-1)/time.z[time.z>0]
...
dim(surv_his)
# surv_his$CESD_log=log(surv_his$CESD+1)
library(survival)
...
#inv_cov_all=list(inv_cov1,inv_cov2,inv_cov3,inv_cov4,inv_cov5,inv_cov)
...
alphas=seq(0.001,0.1,by=0.0005)*1.4
for(jj in 1:90){
